...
pub async fn set_toggl(token: String, name: String, pid: u64) {
    ...
    client.post("https://api.track.toggl.com/api/v8/time_entries/start")
	.header("Content-Type", "application/json")
	.basic_auth(token, Some("api_token"))
	.json(&post)
	.send().await.unwrap();
...
pub async fn update_toggl(token: String, name: String, tags: Vec<String>, pid: u64) {
    ...
    client.put(String::from("https://api.track.toggl.com/api/v8/time_entries/")+&cur["data"]["id"].to_string())
	.header("Content-Type", "application/json")
	.basic_auth(token.clone(), Some("api_token"))
	.json(&post)
	.send().await.unwrap();
